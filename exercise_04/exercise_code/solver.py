...
class Solver(object):
    ...
    def _step(self):
        ...
        y_out = model.forward(X_train) # hat{y}
        loss, loss_gradient = loss_func(y_out, y_train) # L, dL/dhat{y}
        dW = model.backward(y_out) # dhat{y}/dW
        gradient = loss_gradient * dW # dL/dW
