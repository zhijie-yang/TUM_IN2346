...
class Encoder(nn.Module):
    ...
    def __init__(self, hparams, input_size=28 * 28, latent_dim=20):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(input_size, self.hparams["n_hidden_encode_1"]),
            nn.ReLU(),
            nn.Linear(self.hparams["n_hidden_encode_1"], self.hparams["n_hidden_encode_2"]),
            nn.ReLU(),
            nn.Linear(self.hparams["n_hidden_encode_2"], latent_dim)
        )
...
class Decoder(nn.Module):
    ...
    def __init__(self, hparams, latent_dim=20, output_size=28 * 28):
        ...
        self.decoder = nn.Sequential(
            nn.Linear(latent_dim, self.hparams["n_hidden_encode_2"]),
            nn.ReLU(),
            nn.Linear(self.hparams["n_hidden_encode_2"], self.hparams["n_hidden_encode_1"]),
            nn.ReLU(),
            nn.Linear(self.hparams["n_hidden_encode_1"], output_size)
        )
...
class Autoencoder(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        optim = torch.optim.Adam(super().parameters(), self.hparams['learning_rate'])
...
class Classifier(pl.LightningModule):
    ...
    def __init__(self, hparams, encoder, train_set=None, val_set=None, test_set=None):
        ...
        self.model = nn.Sequential(
            nn.Linear(self.encoder.latent_dim, self.hparams["n_hidden_classi"]),
            nn.ReLU(),
            nn.Linear(self.hparams["n_hidden_classi"], self.hparams["num_classes"])
        )
