...
class MyPytorchModel(pl.LightningModule):
    ...
    def __init__(self, hparams, input_size=3 * 32 * 32, num_classes=10):
        ...
        self.model = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(self.hparams["n_hidden_7"], num_classes),
            nn.ReLU(),
            nn.LogSoftmax())
    ...
    def forward(self, x):
        ...
        # x = self.l1(x)
        # x = self.l2(x)
        # x = nn.LogSoftmax(self.l3(x), dim = 0)
        # x = self.l3(x)

        # x = nn.LogSoftmax(self.model(x), dim = 1)
        x = self.model(x)
    ...
    def train_dataloader(self):
        return DataLoader(self.dataset["train"], batch_size=self.hparams["batch_size"], sampler=self.sampler["train"], num_workers=16, drop_last=True)

    def val_dataloader(self):
        return DataLoader(self.dataset["val"], batch_size=self.hparams["batch_size"], sampler=self.sampler["val"], num_workers=16, drop_last=True)

    def test_dataloader(self):
        return DataLoader(self.dataset["test"], batch_size=self.hparams["batch_size"], sampler=self.sampler["test"], num_workers=16, drop_last=True)
